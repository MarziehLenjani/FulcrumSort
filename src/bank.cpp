...
:physicalComponent(l_id, l_confObj, l_firstDimOwner, l_secondDimOwner, l_thirdDimOwner){
	...
	nSubPerBank=l_confObj->getConfig<CONF_NUMBER_OF_COMPUTE_SUBARRAY_PER_BANK_TYPE>(CONF_NUMBER_OF_COMPUTE_SUBARRAY_PER_BANK_NAME);
	for (ID_TYPE i=0;i<nSubPerBank;i++){
